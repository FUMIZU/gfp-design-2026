...
本仓库包含 GFP 设计竞赛参赛作品的完整计算流程，旨在以超折叠 GFP（sfGFP）为序列与结构骨架，通过整合模块化突变组合与生成式序列模型设计具有增强或改变荧光光谱特性的新型绿色荧光蛋白序列，同时保留生色团正常成熟及正确折叠能力。

设计策略包含两条技术路线：其一为基于预定义突变模块在允许位点（残基 1–232，重点覆盖 β-桶框架关键位置）进行系统性组合枚举；其二为基于 GFP 家族序列-结构比对训练生成式模型以提出模块之外的非平凡突变组合，最终输出经混合打分排序的候选序列供下游实验表征。仓库文件夹结构如下：
